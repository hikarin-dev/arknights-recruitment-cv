...
module.exports = async function checkScreenShare(page) {
  ...
  assert.equal(await page.evaluate(() => snapshotsDuringOCR.length >= 10 && snapshotsDuringOCR.every(Boolean)), true, 'old image and results stay intact throughout recognition, including a genuinely new tag set');
