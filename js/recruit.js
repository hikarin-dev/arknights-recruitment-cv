...
let selectedOp;
let screenshotInputActive = false;
...
const recruitmentReady = fetch(`${DATA_BASE[serverString]}/gamedata/excel/gacha_table.json`)
...
function calculateResults() {
	if (Object.keys(TAG_MAP).length === 0) return;
	...
	const ranking = RecruitRanking.rank(groups);
	ranking.best.sort((a, b) => a.tags.length - b.tags.length);
	const recommendation = document.getElementById("recommendation");
	recommendation.hidden = selectedTags.size === 0;
	const labels = { 4: "Guaranteed 6★", 3: "Guaranteed 5★", 2: "4★ or better · 5★ possible", 1: "Guaranteed 4★" };
	const bestNames = ranking.best.map(group => group.tags.map(tag => tag.tagName).join(" + "));
	recommendation.textContent = ranking.best.length
		? `${labels[ranking.tier]}: ${ranking.best.length > 1 ? ranking.best.length + " tied combinations" : bestNames[0]}. Set 9:00; assumes tags stick. Actual rates unknown.`
		: "No combination excludes 3★ at 9:00. No qualifying recommendation.";
	if (ranking.best.length > 1) {
		const label = document.createElement("label");
		label.textContent = " Highlight combination: ";
		const choice = document.createElement("select");
		choice.id = "recommendationChoice";
		bestNames.forEach((name, index) => {
			const option = document.createElement("option");
			option.value = index;
			option.textContent = name;
			choice.appendChild(option);
		});
		choice.onchange = () => document.dispatchEvent(new CustomEvent("recruitment:highlight", { detail: ranking.best[choice.value] }));
		label.appendChild(choice);
		recommendation.appendChild(label);
	}
	document.dispatchEvent(new CustomEvent("recruitment:ranked", { detail: ranking }));
	let fullResultSize = groups.length;
	if (screenshotInputActive) {
		groups = ranking.candidates.map(candidate => ({ ...candidate.group, matches: candidate.eligible, recommended: ranking.best.includes(candidate.group) }));
	} else if (!noobMode) {
	...
	groups.forEach((group) => {
		...
		let tr = document.createElement("tr");
		if (group.recommended || ranking.best.includes(group)) {
			tr.classList.add("recommended-result");
			tr.setAttribute("aria-label", labels[ranking.tier] + " recommended combination");
		}
		let label = document.createElement("td");
		...
		group.matches.forEach((op) => {
			let el = CreateOpCheckbox(op, ops);
	...
	if (screenshotInputActive || !noobMode || !showRobots) {
...
function resetAll() {
	...
	document
		...
		.forEach((el) => el.classList.remove("checked", "highlight", "highlight_low"));
	calculateResults();
}
